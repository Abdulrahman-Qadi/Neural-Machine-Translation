from keras.layers import Embedding,LSTM,Dropout,Dense,Layer,TimeDistributed
...
class AttentionLayer(Layer):
  ...
  def call(self, inputs, mask=None):
    ...
    transposed_decoder_outputs = K.permute_dimensions(decoder_outputs, (0, 2, 1))
    # Computing the dot product
    luong_scores = K.batch_dot(encoder_outputs, transposed_decoder_outputs)
    # Applying softmax to weigh the encoder representations using the scores 
    luong_weights = K.softmax(luong_scores, axis=1)
    # expanding the last dimension to accommodate the hidden_size dimension 
    expanded_luong_scores = K.expand_dims(luong_weights, axis=-1)
    # expanding dimensions so that they have the same shape
    expanded_encoder_outputs = K.expand_dims(encoder_outputs, axis=2)
    # Summing along the dimensions
    encoder_vector = K.sum(expanded_luong_scores * expanded_encoder_outputs, axis=1)
    """
    End Task 3
    """
    # [batch,max_dec,2*emb]
    # Concatenating the weighted encoder with the decoder output
...
class NmtModel(object):
  ...
  def build(self):
    ...
    encoder = LSTM(self.hidden_size, return_sequences=True, return_state=True, dropout=self.embedding_dropout_rate, recurrent_dropout=self.hidden_dropout_rate)

    encoder_outputs, encoder_state_h, encoder_state_c = encoder(source_words_embeddings)
    """
    End Task 1
        """
